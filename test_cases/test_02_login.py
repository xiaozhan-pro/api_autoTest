...
data = excel_handler.sheet_readAll(config.read_yaml()["Excel_handler"]["sheet_login"])  # 从yaml中读取"登录"表单名称
...
class TestLogin(unittest.TestCase):
    ...
    def test_login(self,test_data):
        ...
        try:
            ...
            column_actualResult=excel_handler.sheet_header(config.read_yaml()["Excel_handler"]["sheet_login"]).index("actual_result") #获取"login"表单标题中"actual_result"索引值
            ExcelHandler.sheet_writeCell(config.data_path(),
                                         config.read_yaml()["Excel_handler"]["sheet_login"],
                                         test_data["case_id"]+1,
                                         column_actualResult+1,
                                         str(res))  # 向actual_result写值

            column_testResult=excel_handler.sheet_header(config.read_yaml()["Excel_handler"]["sheet_login"]).index("test_result") #获取"login"表单标题中"test_result"索引值
            ExcelHandler.sheet_writeCell(config.data_path(),
                                         config.read_yaml()["Excel_handler"]["sheet_login"],
                                         test_data["case_id"]+1,
                                         column_testResult+1
