...
data = excel_handler.sheet_readAll(config.read_yaml()["Excel_handler"]["sheet_withdraw"])  # 从yaml中读取"提现"表单名称
...
class TestWithdraw(unittest.TestCase):
    def setUp(self) -> None: #前置条件
        ...
        """充值和登录的用例关联（或者用例依赖）"""
        token = Context.token_withdraw #获取登录时生成的token
        member_id = Context.memberId_withdraw #获取提现ID
    ...
    def test_withdraw(self,test_data):
        ...
        """使用正则表达式匹配excel.withdraw中的 #memberId_withdraw# 、#wrong_memberId_withdraw# ，然后映射Context类对应属性中的值进行替换 """
        ...
        try:
            ...
            column_actualResult=excel_handler.sheet_header(config.read_yaml()["Excel_handler"]["sheet_withdraw"]).index("actual_result") #获取"withdraw"表单标题中"actual_result"索引值
            ExcelHandler.sheet_writeCell(config.data_path(),
                                         config.read_yaml()["Excel_handler"]["sheet_withdraw"],
                                         test_data["case_id"]+1,
                                         column_actualResult+1,
                                         str(res))  # 向actual_result写值

            column_testResult=excel_handler.sheet_header(config.read_yaml()["Excel_handler"]["sheet_withdraw"]).index("test_result") #获取"withdraw"表单标题中"test_result"索引值
            ExcelHandler.sheet_writeCell(config.data_path(),
                                         config.read_yaml()["Excel_handler"]["sheet_withdraw"],
                                         test_data["case_id"]+1,
                                         column_testResult+1,
