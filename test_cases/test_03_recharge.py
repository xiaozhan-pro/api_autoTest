...
data = excel_handler.sheet_readAll(config.read_yaml()["Excel_handler"]["sheet_recharge"])  # 从yaml中读取"充值"表单名称
...
class TestRecharge(unittest.TestCase):
    ...
    def test_recharge(self,test_data):
        ...
        """查询数据库，查询登录后的余额"""
        user = self.db.query("select * from member where id=%s;",args=[self.member_id])
        ...
        try:
            ...
            if res["code"]== 0:
                ...
                """查询数据库，查询充值后的余额"""
                after_user = self.db.query("select * from member where id=%s;", args=[self.member_id])
                after_money = after_user["leave_amount"]

                try:
                    """断言 登录后的余额 + 充值金额 是否等于 充值后的余额"""
                    self.assertEqual(Decimal(str(money)) + Decimal(str(before_money)), Decimal(str(after_money)))
                    """用例测试通过"""
                    test_result = "pass"
            ...
            column_actualResult=excel_handler.sheet_header(config.read_yaml()["Excel_handler"]["sheet_recharge"]).index("actual_result") #获取"recharge"表单标题中"actual_result"索引值
            ...
            column_testResult=excel_handler.sheet_header(config.read_yaml()["Excel_handler"]["sheet_recharge"]).index("test_result") #获取"recharge"表单标题中"test_result"索引值
            ExcelHandler.sheet_writeCell(config.data_path(),
                                         config.read_yaml()["Excel_handler"]["sheet_recharge"],
                                         test_data["case_id"]+1,
                                         column_testResult+1,
