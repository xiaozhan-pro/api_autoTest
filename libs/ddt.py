...
def ddt(cls):
    ...
    For each method decorated with ``@file_data('data.json')``, the
    decorator will try to load the data.json file located relative
    ...
    for name, func in list(cls.__dict__.items()):
        if hasattr(func, DATA_ATTR):
            for i, v in enumerate(getattr(func, DATA_ATTR)):
                ...
                #     test_name = mk_test_name(name, v[3], i)  #源码修改处，测试用例名称加入标题
                # if isinstance(v,dict):
                #     test_name = mk_test_name(name, v["case_name"], i) #源码修改处，测试用例名称加入标题
